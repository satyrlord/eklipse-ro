...
Use this skill only after an explicit quality-gate request. It runs tests, builds, policy checks, and browser checks.
...
1. Read `AGENTS.md`, `PRODUCT.md`, `package.json`, changed files, tests, and `public/.htaccess`.
   Completion criterion: Repository rules, contracts, commands, and gate inputs are available.
...
1. Run `npm test`.
   Completion criterion: The test result is recorded.
...
6. Allow only `https://bandcamp.com/EmbeddedPlayer/` iframe sources for ledger releases.
   Completion criterion: Each iframe source and release classification passes the rule.
...
10. Run the [verify-site](../verify-site/SKILL.md) browser procedure for supported desktop and mobile widths.
...
1. Require direct browser evidence for every changed browser contract.
   Completion criterion: Each changed browser contract has evidence or a blocker.

2. Require a semantic or computed-style assertion for every changed browser contract.
   Completion criterion: A screenshot is not the only evidence for interaction or state.
...
2. Separate failures found before this work from failures caused by the scoped change.
...
3. Report every unavailable browser, hosting, or credential-dependent check.
   Completion criterion: Every unavailable check is marked `BLOCKED` or `N/A`.

4. Do not claim a cPanel result from local build evidence alone.
   Completion criterion: The report separates local evidence from cPanel evidence.

## Completion criterion

The gate is complete when each required check has a result and evidence or an explicit blocker.
