---
name: grill-me
description: "Interview the user one decision at a time to stress-test an eklipse site plan or design before building. Use for grill requests or unresolved product and visual decisions."
argument-hint: "[plan or design to examine]"
---

# Grill Me

Resolve a plan or design by walking its decision tree one branch at a time.

## Rules

1. Explore the repository and official eklipse Bandcamp pages before asking anything they can answer.
2. Ask exactly one question at a time with the VS Code question tool.
3. State the recommended answer and concise tradeoffs before asking.
4. Resolve parent decisions before child decisions.
5. Incorporate each answer immediately and update the relevant durable document.
6. Never reopen a settled branch without new conflicting evidence.

## eklipse Boundaries

- Preserve the static, read-only architecture and Bandcamp-only outbound link allowlist.
- Do not solicit or introduce unsupported commercial claims.
- Treat `PRODUCT.md` as product truth and `DESIGN.md` as visual truth once it exists.

## Completion

Finish when no unresolved dependency would force a builder to reinterpret the design or implementation plan.
