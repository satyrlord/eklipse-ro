...
2. Use `PRODUCT.md`, `.github/copilot-instructions.md`, official Bandcamp pages,
   and current related source as evidence for comparison.
   Completion criterion: Each proposed removal has an evidence source.
3. Mark only content that lacks a valid factual, behavioral, contractual,
   accessibility, or stylistic reason.
...
5. Preserve release facts, album titles, security controls, responsive
   behavior, and outbound-link restrictions.
   Completion criterion: The edit removes no supported content or protected behavior.
6. Validate each edit group with the narrowest relevant build, test, or browser
   check.
...
The cleanup is complete when the report lists edited files, removed-content
categories, excluded files, and validation evidence.
